...
if page == "Dashboard":
    ...
    trend = (
        df.groupby("Date", as_index=False)["Traffic Volume"]
        .mean()
        .set_index("Date")
    )
    st.line_chart(trend)
    ...
    with col2:
        ...
        congestion = df["Congestion Level"].value_counts()
        st.bar_chart(congestion)
    ...
    holiday = st.selectbox(
        "Holiday", list(encoders["Holiday"].classes_)
    )
    ...
    if st.button("🚦 Predict Congestion", type="primary"):
        try:
            ...
            label = encoders["Congestion Level"].inverse_transform(prediction)[0]
